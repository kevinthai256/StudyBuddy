...
// 2. Update local UI and LocalStorage
...
// 3. SYNC TO FIREBASE (The missing step)
...
todos, // Keep current todos

studySessions, // Keep current sessions

events: updatedEvents, // Send the UPDATED events
...
if (!response.ok) throw new Error('Failed to sync deletion');

} catch (error) {

console.error("Error syncing event deletion to Firestore:", error);

// Optional: Rollback local state if the server update fails
